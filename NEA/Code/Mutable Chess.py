...
def vector_to_algebra(coords):
        i = chess_map_from_index_to_alpha[coords[0]]
        j = str(coords[1] + 1)
        pos = i+j
        return pos
...
#updates game info
                                ...
                                self.board = promotion_check(self.board, self.h, self.agents)
...
class Piece:
    ...
    def __init__(self, letter, vectors_ride = None, vectors_leap = None):
        self.piece = letter
        #unique identifier
...
#leaps
        if self.vec_leap:
            ...
            for v in self.vec_leap:
                ...
                for f in operators:
                        for g in operators:
                                moves += leap_move(f, g, v[0], v[1], board, self.piece, column, row)
                                moves += leap_move(f, g, v[1], v[0], board, self.piece, column, row)
...
#rides
        ...
        if self.vec_ride:
            for v in self.vec_ride:
                ...
                for f in operators:
                        for g in operators:
                                moves += ride_move(f, g, v[0], v[1], board, self.piece, column, row)
                                moves += ride_move(f, g, v[1], v[0], board, self.piece, column, row)
...
#check for check       
        if check_check:
            ...
            for i in allPossibleMoves:
                ...
                if not royal_attacked(ChessBoard.getboard(), self.piece, turn, newpos, coords, w, h):
...
class Pawn(Piece):
        ...
        def getMoves(self, pos, board, turn, check_check, w, h): 
                ...
                #black pawn
                if self.piece.islower(): #same as above but in opposite direction
                    if i > 0:
                        try:
                            temp = board[i-1][column]
                            if temp == 1:   
...
white_pawn = Pawn('P')
white_rook = Piece('R', [[0,1]])
white_knight = Piece('N', None, [[1,2]])
white_bishop = Piece('B', [[1,1]])
white_king = Royal('K', None, [[0,1],[1,1]])
white_queen = Piece('Q', [[0,1],[1,1]])
black_pawn = Pawn('p')
black_rook = Piece('r', [[0,1]])
black_knight = Piece('n', None, [[1,2]])
black_bishop = Piece('b', [[1,1]])
black_king = Royal('k', None, [[0,1],[1,1]])
black_queen = Piece('q', [[0,1],[1,1]])

#fairy pieces
# fairy pieces are initialise as none and then set values later if the user decides to use them
white_fairy1 = None
white_fairy2 = None
white_fairy3 = None
black_fairy1 = None
...
fairies = [[white_fairy1, black_fairy1], [white_fairy2, black_fairy2], [white_fairy3, black_fairy3]]
#put into list for easy access
...
pieces = {
          ...
          'X':['white_fairy1',white_fairy1],
          'Y':['white_fairy2',white_fairy2],
          'Z':['white_fairy3',white_fairy3],
          ...
          'x':['black_fairy1',black_fairy1],
          'y':['black_fairy2',black_fairy2],
          'z':['black_fairy3',black_fairy3]
          }
...
#promotion
def promotion_check(chessBoard, h, agents_):
    ...
    for x in range(h):
        for y in range(w):
                ...
                #finds every piece belonging to the attacking player
                        moves.append(pieces[chessBoard[x][y]][1].getMoves((chess_map_from_index_to_alpha[y]+str(x+1)), chessBoard, turn-1, False, w, h))
        ...
        for x in range(h):
                for y in range(w):
                        if type(board[x][y]) == str  and (board[x][y]).isupper() ^ turn-1:
                                move = (pieces[board[x][y]][1].getMoves((chess_map_from_index_to_alpha[y]+str(x+1)), board, turn, True, w, h))
...
#custom ruleset
            ...
            while loop < 3:
                ans = input('Would you like to make a new piece?({}/3):\n'.format(loop+1))
                ...
                if ans.lower() == 'yes':
                    ...
                    if not vec_ride and not vec_leap and define == Piece:
                        ...
                        fairies[loop][0] = define(fairy.upper(), vec_ride, vec_leap)
                        #defines piece using user input

                        pieces[fairy.upper()].pop(-1)
                        #gets rid of previous piece in that piece's slot

                        pieces[fairy.upper()].append(fairies[loop][0])
                        #marks the piece as a fairy

                        #creates an identical black version
                        fairies[loop][1] = define(fairy, vec_ride, vec_leap)

                        pieces[fairy].pop(-1) 

                        pieces[fairy].append(fairies[loop][1])
                        
                        agents += [fairy, fairy.upper()]
...
#play custom             
                ...
                #verifies that they exist
                    ...
                    if choice == 0:
                        ...
                        for i in range(len(custom_pieces)):
                            pieces[letters[i].upper()].pop(-1)
                            pieces[letters[i].upper()].append(custom_pieces[i][0])
                            pieces[letters[i]].pop(-1)
